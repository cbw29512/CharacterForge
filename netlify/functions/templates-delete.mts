...
export default async function deleteTemplate(request: Request): Promise<Response> {
  ...
  try {
    ...
    console.error('CharacterForge template delete failed', error instanceof Error ? error.name : 'unknown_error');
    return json({ error: 'service_unavailable' }, 503);
  }
}

export const config: Config = { path: '/api/templates/delete' };
