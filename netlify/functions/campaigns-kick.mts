...
export default async function kickCampaignMember(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (Number(campaign.rows[0].dm_id) === userId) return json({ error: 'owner_membership_required' }, 409);
    ...
    console.error('CharacterForge campaign kick failed', error instanceof Error ? error.name : 'unknown_error');
    return json({ error: 'service_unavailable' }, 503);
  }
}

export const config: Config = { path: '/api/campaigns/kick' };
